...
const teachers = [
    ...
    _portraits = createIndexFile_PSPA(numberTotalStudents,3,2);
...
function createIndexFile_PSPA (numTotal,grade,numTeachers=1) {


    let portraits = [];
    let teacherGroups = numTeachers=0 ? 1 : numTeachers;
    let studentsPerTeacher = numTeachers=0 ? numTotal : Math.floor(numTotal / numTeachers)
    //TODO: pick up the mod result

    console.log(studentsPerTeacher);
    for (let t = 0; t<teacherGroups;t++) {
        let gradeTeacher = numTeachers>0 ? teachers[numTeachers-1][t] : '';
        for (let i = 0; i < studentsPerTeacher; i++) {
            ...
            p.grade = grade;
            p.homeRoom = gradeTeacher+'_'+grade;
            p.period = '3';
            p.teacherName = gradeTeacher;
            p.track = '';
            p.department = '';
